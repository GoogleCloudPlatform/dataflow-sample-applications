...
public abstract class GraphType1Comp implements Serializable {
  ...
  public PCollection<KV<TSKey, TSAccum>> genType1ComputationGraph(PCollectionTuple allDataTypes) {
    ...
    int hotKeyfanout = Optional.ofNullable(getGenerateComputations().hotKeyFanOut()).orElse(5);

    for (CombineFn<TSDataPoint, TSAccum, TSAccum> combine :
        getGenerateComputations().type1NumericComputations()) {
