...
> Note: All data is sent as JSON, this is normal for clickstream data but not necessarily the norm for Transaction / Stock data. Different data formats (e.g xml)  will be supported in future versions.
...
There is a utility class in Beam JsonToRow, while in 2.22.0 there are limitations with this utility:
Can not deal with nulls
No easy way to build in dead letter
However these limitations are being worked on and with these additions, JsonToRow will become one of the primary recommended solutions for converting Json to either Row or Pojo objects. If there is a need to convert a JSON object to a AutoValue POJO then the Convert utility class can be used, which created the flow:

Read -> JsonToRow -> Convert.fromRows()
Example of usage

>Note: For v1.0 of the retail application we have created local copies of JsonToRow to incorporate some of the changes from Beam master not available in 2.22.0. 

ProcessClickstream.java
JSONUtils.java
Deadletter pattern
...
##Data corrections & external service calls 
### Why needed?
When attempting to repair data flowing within a streaming pipeline there are generally two forms that can be present:
Data that can be corrected using rules that dont need external data sources. 
Data that needs information not available in the pipeline, often requiring a call to a data store or an external service API which can correct the data point.
For item 1 the changes can be coded within transform(s), however for item 2 we need to make calls out to other services. 
### What is a good solution?
...
This has lots of control in the exact size of the batch that is created.
* Cons

Requires a shuffle, which will be done on an arbitrary key space. Magic number time!
...
bundle sizes are non-deterministic, they are based on the runners implementations and often in stream mode will be very small in size.
...
##Use of schemas for analytical calls.
###Why needed?
