...
public class CountGlobalStockFromTransaction
  ...
  public PCollection<StockAggregation> expand(PCollection<StockAggregation> input) {
    return input
        .apply("SelectProductId", Select.<StockAggregation>fieldNames("productId"))
        .apply(
            Group.<Row>byFieldNames("productId")
                .aggregateField("productId", Count.combineFn(), "count"))
        .apply("SelectProductCount", Select.fieldNames("key.productId", "value.count"))
        .apply(
            AddFields.<Row>create()
                .field("storeId", FieldType.INT32)
